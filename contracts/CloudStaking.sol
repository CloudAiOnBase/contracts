...
contract CloudAIStaking is Ownable {
    ...
    uint256 public minStakeAmount;                  // e.g. 100 CLOUD (in token smallest units)
    uint256 public cooldown;                        // 10 days cooldown for unstaking
    uint256 public governanceInactivityThreshold;   // 1 year inactivity threshold (for governance tally)
    uint256 public autoUnstakePeriod;               // 3 years auto-unstake for inactive stakers
    uint256 public aprMin;                          // APR range minimum (4%)
    uint256 public aprMax;                          // APR range maximum (10%)
    uint256 public stakedCircSupplyMin;             // 10%
    uint256 public stakedCircSupplyMax;             // 50%
    uint256 public cachingPeriod;                   // 24 hours
    uint256 public lastParamsUpdate;                // Caching period for external parameters
